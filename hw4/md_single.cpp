...
vector<coordinate_vector> forces(const vector<coordinate_vector> &pos_mat) {
  assert(pos_mat.size()%BatchSize == 0);
  ...
  vector<force_thread_arg_t> force_thread_args(pos_mat.size()/BatchSize);
  ...
  assert(force_thread_args.front().range_begin == 0);
  assert(force_thread_args.back().range_end == pos_mat.size());

  for (unsigned int i=0; i<force_thread_args.size(); ++i) {
    force_thread(&force_thread_args[i]);
  }

  return result_vec;
}

vector<coordinate_vector> acceleration(const vector<coordinate_vector> &force_mat,
                                       const vector<double> &mass_mat) {
  assert(force_mat.size()==mass_mat.size());
  vector<coordinate_vector> accel_mat(force_mat.size());
  for (unsigned int i=0; i<accel_mat.size(); ++i) {
    accel_mat[i].resize(force_mat[i].size());
    for (unsigned int j=0; j<force_mat[i].size(); ++j) {
      accel_mat[i][j] = force_mat[i][j]/mass_mat[i];
    }
  }
  return accel_mat;
}
...
void velocity_verlet(vector<coordinate_vector> &pos_mat,
  ...
  vector<coordinate_vector> accel_mat = acceleration(forces(pos_mat), mass_mat);
  for (unsigned int i=0; i<pos_mat.size(); ++i) {
    for (unsigned int j=0; j<pos_mat[i].size(); ++j) {
  ...
  vector<coordinate_vector> new_accel_mat = acceleration(forces(pos_mat), mass_mat);
...
int main() {
  ...
  double my_box=15;
  unsigned int TotalStep=10000;
  unsigned int OutputInterval=5;
  double steptime = 1e-2;
