...
double  BoxSize,                        // Half size of simulated box, simulation space is [-BoxSize,BoxSize]
  ...
  for (size_t i=0; i < pos_mat.size(); ++i) {
    for (size_t j=i+1; j < pos_mat.size(); ++j) {
      ...
      atom_type_t i_type = type_vec[i],
                  j_type = type_vec[j];
      pot += 4*LJEplison[i_type][j_type] * ( pow(LJSigma[i_type][j_type]*LJSigma[i_type][j_type]/dist2, 6) - pow(LJSigma[i_type][j_type]*LJSigma[i_type][j_type]/dist2, 3) );
    }
  }
  return pot;
}
...
vector<coordinate_vector> acceleration(const vector<coordinate_vector> &pos_mat,
  ...
  for (unsigned int i=0; i<NThread; ++i) {
    ...
    for (size_t j=force_thread_args[i].range_begin; j<force_thread_args[i].range_end; ++j) {
      for (unsigned int k=0; k<DIMENSION; ++k) {
        result_vec[j][k] /= Mass[type_vec[j]];
      }
    }
  }
  return result_vec;
...
void simulate(vector<coordinate_vector> &pos_mat,
              vector<coordinate_vector> &vel_mat,
              const vector<atom_type_t> &type_vec) {
  print_pos(pos_mat, energy(pos_mat, vel_mat, type_vec), 0);
  for (size_t i=1; i<=TotalSteps; ++i) {
    velocity_verlet(pos_mat, vel_mat, type_vec);
    if (i%OutputInterval==0) {
      print_pos(pos_mat, energy(pos_mat, vel_mat, type_vec), i);
    }
  }      
}
